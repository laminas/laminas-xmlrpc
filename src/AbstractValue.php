...
abstract class AbstractValue
{
    ...
    public static function getXmlRpcValue(
        ...
        return match ($type) {
            ...
            self::AUTO_DETECT_TYPE => static::phpVarToNativeXmlRpc($value),

            // Parse the XML string given in $value and get the XML-RPC value in it
            self::XML_STRING => static::xmlStringToNativeXmlRpc($value, $libXmlOptions),

            // fall through to the next case
            self::XMLRPC_TYPE_I4,
            self::XMLRPC_TYPE_INTEGER =>
                new Value\Integer($value),

            // fall through to the next case
            self::XMLRPC_TYPE_I8,
            self::XMLRPC_TYPE_APACHEI8 =>
                self::$USE_BIGINT_FOR_I8 ? new Value\BigInteger($value) : new Value\Integer($value),

            self::XMLRPC_TYPE_DOUBLE => new Value\Double($value),

            self::XMLRPC_TYPE_BOOLEAN => new Value\Boolean($value),

            self::XMLRPC_TYPE_STRING => new Value\Text($value),

            self::XMLRPC_TYPE_BASE64 => new Value\Base64($value),

                // fall through to the next case
            self::XMLRPC_TYPE_NIL,
            self::XMLRPC_TYPE_APACHENIL => new Value\Nil(),

            self::XMLRPC_TYPE_DATETIME => new Value\DateTime($value),

            self::XMLRPC_TYPE_ARRAY => new Value\ArrayValue($value),

            self::XMLRPC_TYPE_STRUCT => new Value\Struct($value),

            default => throw new ValueException('Given type is not a ' . self::class . ' constant'),
        };
    }
    ...
    {
        return match (true) {
            is_object($value) => (function () use ($value) {
            ...
            is_int($value) => $value > PHP_INT_MAX ? self::XMLRPC_TYPE_I8 : self::XMLRPC_TYPE_INTEGER,
            is_float($value) => self::XMLRPC_TYPE_DOUBLE,
            is_bool($value) => self::XMLRPC_TYPE_BOOLEAN,
            null === $value => self::XMLRPC_TYPE_NIL,
            is_string($value) => self::XMLRPC_TYPE_STRING,
    ...
    protected static function phpVarToNativeXmlRpc(mixed $value): AbstractValue
    {
        ...
        return match (static::getXmlRpcTypeByValue($value)) {
            ...
            self::XMLRPC_TYPE_ARRAY =>
                /** @psalm-suppress MixedArgument */
                 new Value\ArrayValue($value),

            self::XMLRPC_TYPE_STRUCT =>
                /** @psalm-suppress MixedArgument */
                 new Value\Struct($value),

            self::XMLRPC_TYPE_INTEGER =>
                /** @psalm-suppress MixedArgument */
                 new Value\Integer($value),

            self::XMLRPC_TYPE_DOUBLE =>
                /** @psalm-suppress MixedArgument */
                 new Value\Double($value),

            self::XMLRPC_TYPE_BOOLEAN =>
                /** @psalm-suppress MixedArgument */
    ...
    protected static function xmlStringToNativeXmlRpc(
        ...
        $xmlrpcValue = match ($type) {
            ...
            self::XMLRPC_TYPE_STRING =>
                new Value\Text($value),
            self::XMLRPC_TYPE_DATETIME =>  // The value should already be in an iso8601 forma =>
                new Value\DateTime($value),
            self::XMLRPC_TYPE_BASE64 =>    // The value should already be base64 encode =>
                new Value\Base64($value, true),
            self::XMLRPC_TYPE_NIL,
                // Fall through to the next case
            self::XMLRPC_TYPE_APACHENIL =>
                // The value should always be NULL
                new Value\Nil(),
            self::XMLRPC_TYPE_ARRAY => (function () use ($value, $libXmlOptions) {
            ...
            self::XMLRPC_TYPE_STRUCT => (function () use ($value, $libXmlOptions) {
                $values = [];
                ...
                return new Value\Struct($values);
            })(),
            default =>
                throw new ValueException(
